...
class WonderPlugin_Carousel_Model {
	...
	function generate_body_code($id, $has_wrapper) {
		...
		if ($item_row != null)
		{
			...
			$boolOptions = array('fixaspectratio', 'autoplay', 'random', 'autoplayvideo', 'circular', 'pauseonmouseover', 'continuous', 'responsive', 'showhoveroverlay', 'showhoveroverlayalways', 'hidehoveroverlayontouch', 'lightboxresponsive', 'lightboxshownavigation', 'lightboxnogroup', 'lightboxshowtitle', 'lightboxshowdescription', 'usescreenquery', 'donotinit', 'addinitscript');
			...
			$valOptions = array('rownumber', 'visibleitems', 'arrowstyle', 'arrowimage', 'arrowwidth', 'arrowheight', 'navstyle', 'navimage', 'navwidth', 'navheight', 'navspacing', 'hoveroverlayimage', 'lightboxthumbwidth', 'lightboxthumbheight', 'lightboxthumbtopmargin', 'lightboxthumbbottommargin', 'lightboxbarheight', 'lightboxtitlebottomcss', 'lightboxdescriptionbottomcss', 'continuousduration', 
					'scrollmode', 'interval', 'transitionduration');
			...
			if (isset($data->slides) && count($data->slides) > 0)
			{
				...
				{		
					...
					if ($slide->type == 6)
					{
						...
						$ret .= $this->create_post_code($items, $data);
					}
					else
					{
						if ($count == 0)
	...
	function get_post_items($options) {
		...
		foreach($posts as $post)
		{
			...
			{
				$featured_image = wp_get_attachment_image_src(get_post_thumbnail_id($post['ID']), $options->featuredimagesize);
	...
	function create_post_code($slides, $data) {
		...
		foreach($slides as $slide)
		{
			...
			$skin_template = str_replace('__IMAGE__',  $image_code, $skin_template);
			$skin_template = str_replace('__TITLE__',  $slide->title, $skin_template);
